...
var applyRepoWebhooksCmd = &cobra.Command{
	Use:   "repo-webhooks",
	Short: "Create or update multiple repository webhooks from a YAML file",
	Long: `Create or update multiple repository webhooks from a YAML file.
		...
		for i := 0; i < workers; i++ {
			...
			go func() {
				...
				for wh := range ch {
					...
					webhookID, err := internal.FindWebhookIDByName(ctx, client, projectKey, wh.RepoSlug, name)
					if err != nil {
						utils.Logger.Error("Failed to find existing webhook", "repo", wh.RepoSlug, "name", name, "error", err)
						continue
					}
					...
					if webhookID != "" {
						// Update webhook
						_, _, err := client.RepositoryAPI.UpdateWebhook1(ctx, projectKey, webhookID, wh.RepoSlug).
							RestWebhook(req).
							Execute()
						if err != nil {
							utils.Logger.Error("Failed to update repository webhook", "repo", wh.RepoSlug, "name", name, "error", err)
							continue
						}
						utils.Logger.Info("Repository webhook updated", "project", projectKey, "repo", wh.RepoSlug, "name", name)
					} else {
						// Create webhook
...
func init() {
	applyRepoWebhooksCmd.PersistentFlags().StringVarP(&webhooksFile, "file", "f", "", "YAML file with repository webhooks")

	applyRepoWebhooksCmd.PersistentFlags().StringVar(&globalWebhookName, "name", "", "Global webhook display name (used if missing in YAML)")
	applyRepoWebhooksCmd.PersistentFlags().StringVar(&globalWebhookURL, "url", "", "Global webhook target URL (used if missing in YAML)")
	applyRepoWebhooksCmd.PersistentFlags().StringSliceVar(&globalWebhookEvents, "events", nil, "Global comma-separated list of webhook events (used if missing in YAML)")
	applyRepoWebhooksCmd.PersistentFlags().BoolVar(&globalWebhookActive, "active", true, "Global active state for webhook (used if missing in YAML)")
	applyRepoWebhooksCmd.PersistentFlags().StringVar(&globalWebhookUsername, "username", "", "Global HTTP basic auth username (used if missing in YAML)")
	applyRepoWebhooksCmd.PersistentFlags().StringVar(&globalWebhookPassword, "password", "", "Global HTTP basic auth password (used if missing in YAML)")
	applyRepoWebhooksCmd.PersistentFlags().BoolVar(&globalWebhookSSLVerify, "ssl-verification", true, "Global SSL verification flag (used if missing in YAML)")
}
